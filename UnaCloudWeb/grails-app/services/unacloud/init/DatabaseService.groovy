...
class DatabaseService {
	...
	def initDatabase(){
		...
		try{
			sql.execute ('IF NOT EXISTS(SELECT 1 FROM mysql.proc p WHERE name = \'sp_check_pm\') THEN '+
			'CREATE PROCEDURE sp_check_pm BEGIN UPDATE physical_machine SET state = \'OFF\' where CURRENT_TIMESTAMP > DATE_ADD(last_report, INTERVAL 4 MINUTE) AND state = \'ON\'; END'+
			'END IF;')
		...
		try{
			sql.execute ('IF NOT EXISTS(SELECT 1 FROM mysql.proc p WHERE name = \'sp_check_vm\') THEN '+
			'CREATE PROCEDURE sp_check_vm BEGIN UPDATE virtual_machine_execution SET status = \'RECONNECTING\' where CURRENT_TIMESTAMP > DATE_ADD(last_report, INTERVAL 4 MINUTE) AND status = \'DEPLOYED\';'+ 
