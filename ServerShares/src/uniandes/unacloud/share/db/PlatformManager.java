...
public class PlatformManager {
	...
	public static List<PlatformEntity> getAll(Connection con){
		try {
			...
			while(rs.next())list.add(new PlatformEntity(rs.getLong(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6)));
	...
	public static PlatformEntity getPlatform(int id, Connection con){
		try {
			...
			if(rs.next())repo = new PlatformEntity(rs.getLong(1), rs.getString(3), rs.getString(2), rs.getString(4), rs.getString(5), rs.getString(6));
			try{rs.close();ps.close();}catch(Exception e){}
			return repo;
		} catch (Exception e) {
			e.printStackTrace();
