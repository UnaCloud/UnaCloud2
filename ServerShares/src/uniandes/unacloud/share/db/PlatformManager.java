...
public class PlatformManager {
	...
	public static List<PlatformEntity> getAll(Connection con){
		try {
			...
			String query = "SELECT hv.id, hv.platform_version, hv.name, hv.main_extension, hv.files_extensions FROM platform hv;";
	...
	public static PlatformEntity getPlatform(int id, Connection con){
		try {
			PreparedStatement ps = con.prepareStatement("SELECT re.id, re.name, re.platform_version, re.main_extension, re.files_extensions FROM platform re WHERE re.id = ?;");
			ps.setLong(1, id);
			System.out.println(ps.toString());
			ResultSet rs = ps.executeQuery();	
			PlatformEntity repo = null;
			if(rs.next())repo = new PlatformEntity(rs.getLong(1), rs.getString(3), rs.getString(2), rs.getString(4), rs.getString(5));
			try{rs.close();ps.close();}catch(Exception e){}
			return repo;
		} catch (Exception e) {
			e.printStackTrace();
