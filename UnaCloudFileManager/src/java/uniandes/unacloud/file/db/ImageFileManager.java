...
public class ImageFileManager {
	...
	public static boolean setImageFile(ImageFileEntity image, boolean update,Connection con, boolean withToken){
		...
		try {
			...
			if(!withToken&&image.getToken()!=null){query+=(isPublic>0||repository>0||mainFile>0||token>0?",":"")+" vm.token = ? ";token = status>0?status+1:mainFile>0?mainFile+1:repository>0?repository+1:isPublic+1;}
	...
	public static List<ImageFileEntity> getAllImagesByUser(Long userId,Connection con){
		try {
			...
			System.out.println(ps.toString());
			ResultSet rs = ps.executeQuery();		
			while(rs.next())list.add(new ImageFileEntity(rs.getLong(1), ImageEnum.getEnum(rs.getString(2)), rs.getString(3), StorageManager.getRepository(rs.getLong(4),con),PlatformManager.getPlatform(rs.getInt(9),con), rs.getBoolean(5), rs.getLong(6), rs.getString(7), rs.getString(8),null));
			try{rs.close();ps.close();}catch(Exception e){}
			return list;
		} catch (Exception e) {
			e.printStackTrace();
